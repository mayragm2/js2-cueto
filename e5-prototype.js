class Animal {
  ...
  eat(amount) {
    ...
    if (length > this.energy){
        console.log ("Your animal does not have enough energy.");
    }
}
}
...
class Cat extends Animal {
    ...
    play (length){
    if (length > this.energy){
        console.log ("Your cat does not have enough energy.");
    } else {
    this.energy -= length;
    console.log (`Your cat, ${this.name} has played for ${length} minutes. It has ${this.energy} energy points remaining.`);
    }}}

const d1 = new Dog ("Rex", 7);
const c1 = new Cat ("Miau", 10);
